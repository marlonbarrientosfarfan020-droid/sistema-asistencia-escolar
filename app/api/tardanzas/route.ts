...
function minutosEntreHoras(
  ...
  const [hora, minuto] = horaProgramada.split(":").map(Number);

  const entradaProgramada = new Date(horaEntradaReal);
  entradaProgramada.setHours(hora, minuto, 0, 0);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const detalle = tardanzas.map((registro) => ({
      id: registro.id,
      ...
      estudiante: {
        ...
        turno:
          registro.estudiante.turno?.nombre ||
          "Sin turno",
      },
      minutosTardanza: minutosEntreHoras(
        registro.horaEntrada,
        registro.estudiante.turno?.horaEntrada ||
          "00:00"
      ),
    }));
    ...
    console.error("Error obteniendo tardanzas:", error);

    return NextResponse.json(
      {
        ok: false,
        message: "Error al obtener tardanzas",
      },
      {
        status: 500,
      }
